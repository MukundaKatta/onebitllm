...
def test_accuracy_at_scale():
    c = Onebitllm()
    for i in range(500):
        r = c.quantize_layer(i=i)
        assert r["ok"] is True
        assert r["n"] == i + 1

def test_reset_is_instant():
    c = Onebitllm()
    for _ in range(100): c.quantize_layer()
    start = time.time()
    c.reset()
    elapsed = time.time() - start
    assert elapsed < 0.01
...
def test_memory_stable():
    ...
    c = Onebitllm()
    for _ in range(1000): c.quantize_layer(data="x" * 100)
    c.reset()
    assert c.get_stats()["ops"] == 0
