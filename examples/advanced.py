...
def main():
    instance = Onebitllm(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced onebitllm Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.quantize_layer(input=inp, index=i) for i, inp in enumerate(inputs)]
    ...
    start = time.time()
    for i in range(1000):
        instance.quantize_layer(iteration=i)
